...
1. A *textually inline* symbol's definition is exposed to the reader at
   the point of declaration.
2. A function or variable defined in a header file is *expandable inline*
...
3. *ODR-safe* entities do not violate the
   ["One Definition Rule"](https://en.cppreference.com/w/cpp/language/definition),
   which often requires the [inline](https://en.cppreference.com/w/cpp/language/inline)
...
[private](#Access_Control) section of a class, within a namespace that includes the word
...
1. `dir2/foo2.h`.
2. A blank line
3. C system headers, and any other headers in angle brackets with the
...
- To place generated protocol
  ...
  `.proto` file. See
  [Protocol Buffer Packages](https://protobuf.dev/reference/cpp/cpp-generated/#package)
  for details.
...
- Do not use *Namespace aliases* at namespace scope
  ...
  they must have [appropriate names](#Naming_Aliases).
...
Functions in Header Files](#Defining_Functions_in_Header_Files) for more details.
...
See [TotW #148](https://abseil.io/tips/148) for more.
...
- When printing values, use type-safe numeric formatting libraries like
  [`absl::StrCat`](https://github.com/abseil/abseil-cpp/blob/master/absl/strings/str_cat.h),
  [`absl::Substitute`](https://github.com/abseil/abseil-cpp/blob/master/absl/strings/substitute.h),
  [`absl::StrFormat`](https://github.com/abseil/abseil-cpp/blob/master/absl/strings/str_format.h),
  or [`std::ostream`](#Streams) instead of the
  `printf` family of functions.
- When moving structured data into or out of your process, encode it using a
  serialization library like
  [Protocol
  Buffers](https://protobuf.dev/) rather than copying the in-memory representation around.
- If you need to work with memory addresses as integers, store them in
  `uintptr_t`s rather than `uint32_t`s or
  `uint64_t`s.
- Use [braced-initialization](#Casting) as needed to create
  ...
  ```
- Use portable [floating point types](#Floating-Point_Types);
  avoid `long double`.
- Use portable [integer types](#Integer_Types); avoid
...
[Designated initializers](https://en.cppreference.com/w/cpp/language/aggregate_initialization#Designated_initializers) are a syntax that allows for initializing an
...
[promise type](https://en.cppreference.com/w/cpp/language/coroutines#Promise)) which determines coroutine parameter types, how coroutines
...
([even
when singular](https://apastyle.apa.org/style-grammar-guidelines/grammar/singular-they)), and "it"/"its" for software, computers, and other
...
has a dedicated
[page
in a devguide](https://abseil.io/docs/cpp/guides/status),
...
[naming constants](#Constant_Names).
...
see [Storage Duration](http://en.cppreference.com/w/cpp/language/storage_duration#Storage_duration) for details) should be named this way, including those that are static constant
...
When [choosing names](#General_Naming_Rules) for namespaces, note
that names must be fully qualified when used in a header outside the namespace,
because [unqualified Aliases are generally banned](#Aliases).
...
(see [TotW #130](https://abseil.io/tips/130)).

### Enumerator Names

Enumerators (for both scoped and unscoped enums) should be named like
[constants](#Constant_Names), not like
[macros](#Macro_Names). That is, use `kEnumName` not
`ENUM_NAME`.
...
[types](#Type_Names), and non-type template
parameters should follow the rules for naming [variables](#Variable_Names) or [constants](#Constant_Names).

### Macro Names

You're not really going to [define a macro](#Preprocessor_Macros), are you? If you do, they're like this:
...
The name for an [alias](#Aliases) follows the same principles as
...
- a string literal that cannot easily be wrapped at 80 columns.
  ...
  [disable the tool](https://clang.llvm.org/docs/ClangFormatStyleOptions.html#disabling-formatting-on-a-piece-of-code) around the content as necessary.
...
the [line length limit](#Line_Length), or to attach a comment to only
