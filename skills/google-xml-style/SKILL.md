...
description: >-
  Use when designing or reviewing XML document formats under Google's naming, structure,
  extensibility, namespace, validation, and schema conventions. Do not use merely to
  pretty-print an existing XML file.
---
...
2. The schema language SHOULD be [RELAX NG](http://www.relaxng.org/ "RELAX NG") [compact syntax](http://www.relaxng.org/compact-tutorial-20030326.html "compact syntax").  Embedded [Schematron](http://www.schematron.com/ "Schematron") rules MAY be added to the schema for additional fine control.  [*Rationale:*
...
1. Element names MUST be in a namespace, except
...
3. Namespace names are HTTP URIs.  Namespace names SHOULD take the form https://example.com/*whatever*/*year,* where *whatever* is a unique value based on the name of the document type, and *year*
...
2. Names MUST contain only ASCII letters and digits.
...
4. Published standard abbreviations, if sufficiently well-known, MAY be employed in constructing names. Ad hoc abbreviations MUST NOT be used.  Acronyms MUST be treated as words for camel-casing purposes: informationUri, not informationURI. [*Rationale:*  An abbreviation that is well known
...
3. Dates should be represented using [RFC 3339](https://www.ietf.org/rfc/rfc3339.txt "RFC 3339") format, a subset of both
...
2. If the number of possible keys is very large or unbounded, key-value pairs MAY be represented by a single generic element with key, value, and optional unit and scheme
...
5. Redundant whitespace in a tag SHOULD NOT be
...
8. Attribute values MAY be surrounded with either quotation marks or apostrophes.
...
10. If comments are nevertheless used, they SHOULD appear only in the document prolog or in elements that
