...
You should follow the rules for [Spelling and Abbreviations](#Spelling_and_Abbreviations)
...
See the section on [Spelling and Abbreviations](#Spelling_and_Abbreviations)
...
[the CLtL2 chapter on predicates](http://www.cs.cmu.edu/Groups/AI/html/cltl/clm/node69.html).
...
- Intended to be included
  in the `:use` specification of other packages.
...
- Not intended to be "used".
  To reference a facility provided by package `B`,
...
> The fact is, `(declare (type ...))` does different things
...
> That is, if some variable has the value `1432` in it,
...
or an
[Interface-Passing Style](https://common-lisp.net/~frideau/lil-ilc2012/lil-ilc2012.html) interface
...
- Some variables may hold dictionaries
  ...
  you can sometimes use `LOAD-TIME-VALUE` for good effect.
...
- Some variables may hold temporary data
  ...
  Predefined such variables would include `*readtable*`
...
[Using Libraries](#Using_Libraries) and
...
short of `UNINTERN`ing the symbol
...
or `(otherwise nil)` as the last in your CASE,
...
any such [unsafe operations](#Unsafe_Operations).
...
Libraries such as
[wu-decimal](http://wukix.com/lisp-decimals)
may help you;
...
1. There is some good reason to think that the overall effect
   on performance is noticeable, and
2. It is absolutely clear that the assertion is true.
3. It is quite unlikely that the code will be changed
...
[lambda-reader](http://cliki.net/lambda-reader),
...
`LOGICAL-PATHNAME`s are not a portable abstraction,
...
you cannot rely on the test for `INTEGER`ness
...
in the discussion about [EVAL](#EVAL).
