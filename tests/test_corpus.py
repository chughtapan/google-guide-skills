...
def test_release_version_and_generated_provenance_are_aligned() -> None:
    ...
    for source_path in sorted((ROOT / "skills").glob("*/references/source.json")):
        provenance = json.loads(source_path.read_text(encoding="utf-8"))
        assert provenance["generated_by"] == f"google-guide-skills/{version}"


def test_json_skill_ships_both_scoped_license_texts() -> None:
    skill = ROOT / "skills" / "google-json-style" / "references"
    provenance = json.loads((skill / "source.json").read_text(encoding="utf-8"))
    supplemental = provenance["supplemental_licenses"]
    assert [item["spdx"] for item in supplemental] == ["Apache-2.0"]
    apache = skill / "LICENSE-Apache-2.0.txt"
    assert hashlib.sha256(apache.read_bytes()).hexdigest() == supplemental[0]["sha256"]
    assert "code samples" in provenance["license_note"]
